...
export function PasswordRecoveryTemplate({ domain, token, metadata }: PasswordRecoveryTemplateProps) {
	const resetLink = `${domain}/auth/recovery/${token}`
			...
			<Tailwind>
				<Body className='max-w-2xl mx-auto p-6 bg-slate-50'>
					<Section className='text-center mb-8'>
						...
						<Link href={resetLink} className='inline-flex justify-center items-center rounded-full text-sm font-medium text-white bg-[#3870FF] px-5 py-2'>
							Сбросить пароль
						</Link>
					</Section>
					...
					<Section className='bg-gray-100 rounded-lg p-6 mb-6'>
						<Heading 
							className='text-xl font-semibold text-[#3870FF]'
						>
					...
					<Section className='text-center mt-8'>
						<Text className='text-gray-600'>
							...
							<Link 
								...
								className="text-[#3870ff] underline"
							>
								help@gd-tavern.ru
							</Link>.
						</Text>
